...
class KnowledgeGraph:
    ...
    def search(self, query: str, tags: List[str] = None, limit: int = 10) -> List[Dict]:
        with self._lock:
            ...
            results_with_idx = [(score, -node.get("access_count", 0), idx, node)
                                for idx, (score, node) in enumerate(results)]
            return [n for _, _, _, n in sorted(results_with_idx, reverse=True)[:limit]]
...
class MultiAgentFleet:
    ...
    def select_best(self, task_description: str) -> Optional[Dict]:
        ...
        # Tokenize: split on spaces/punctuation for better matching
        tokens = set(re.split(r'[\s\-_/]+', d))
        d_set = set(re.split(r'[\s\-_/]+', d))
        ...
        for agent in self._agents.values():
            ...
            token_matches = len(caps & d_set)
            score += token_matches * 3
            
            # Substring matches (medium weight)
            for cap in caps:
                if cap in d and cap not in d_set:
                    score += 1
            
            # Keyword fuzzy matches (lower weight)
            for token in tokens:
...
''',
        ...
        "rest_api": {
            "file": "api_server.py",
            "template": '''#!/usr/bin/env python3
"""REST API Server — Generated by NEXUS OS (stdlib only, no dependencies)"""
import json
import time
        ...
        if main_file.suffix == ".py":
            try:
                ...
                if syntax_check.returncode != 0:
                    return {
                        "success": False,
                        "error": f"Syntax error: {syntax_check.stderr[:200]}",
                        "syntax_check": "failed",
                    }
                ...
                if main_file.name == "api_server.py":
                    ...
                    if "healthy" in result.stdout:
                        return {
                            "success": True,
                            "stdout": "REST API server started successfully — /health returns healthy",
                    ...
                    return {
                        "success": False,
                        "error": f"Server did not start: {check.stderr[:200]}",
                        "returncode": check.returncode,
                    }
                
                # CLI tools: help output is success
                if main_file.name == "cli_tool.py":
                    result = subprocess.run(
                ...
                if main_file.name == "processor.py":
                    ...
                    has_output = (project_path / "output.json").exists()
                    return {
                        ...
                        "returncode": result.returncode,
                        "type": "data_processor",
                        "output_created": has_output,
                    }
                
                # Web scraper / general: run with timeout
                result = subprocess.run(
                    [sys.executable, str(main_file)],
